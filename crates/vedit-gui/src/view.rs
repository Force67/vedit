...
fn render_command_palette_contents(state: &EditorState) -> Element<'_, Message> {
...
fn render_command_palette(state: &EditorState) -> Element<'_, Message> {
    ...
    // NOTE: Previously this returned a full-screen container with `.style(overlay_container())`
    // which painted a dimming backdrop and replaced the entire main UI.
    // We now simply center the dropdown without any overlay style.
    container(dropdown)
        .width(Length::Fill)
        .height(Length::Fill)
        .center_x()
